...
// Last update Thu May 22 16:23:08 2014 jonathan
//
...
Loader::Loader()
{
  ...
  _models[WallType] = new gdl::Model();
  ...
  _textures[WallType] = new gdl::Texture();
...
bool	Loader::loadTextures()
{
  ...
  std::cout << ".";
  if (_textures[GroundType]->load("./assets/floor.tga") == false)
    {
      std::cerr << "Cannot load the Ground texture" << std::endl;
      return (false);
    }
   std::cout << ".";
  if (_textures[WallType]->load("./assets/wall.tga") == false)
    {
